# train_gru_sac.py
# Trains GRU+SAC variant (V6)
# Drop-in replacement of Transformer with GRU
# Used for ablation: is Transformer better than GRU?

import config
config.TASCAR_MODEL_PATH = "trained_model_gru_sac/"
config.TASCAR_RESULTS    = "results_gru_sac/"
...
def train_gru_sac():
    ...
    print(f"\nGRU+SAC Configuration:")
    print(f"  State dim:    {state_dim}")
    print(f"  Action dim:   {action_dim}")
    print(f"  Encoder:      GRU (bidirectional, 2 layers)")
    print(f"  Episodes:     {TASCAR_EPISODES}")
    print(f"  Random seed:  {RANDOM_SEED}")

    # Use GRU encoder instead of Transformer
    ...
    for ep in range(20):
        ...
        call_count = wmt_before = step_cold = step_warm = 0
        ...
        for call in episode_calls:
            ...
            if call_count % TASCAR_DELTA == 0:
                ...
                next_enc = gru(
                    torch.FloatTensor(
                        history.get_sequence()).unsqueeze(0))
    ...
    for episode in range(1, TASCAR_EPISODES + 1):
        ...
        encoded_state = gru(
            torch.FloatTensor(
                history.get_sequence()).unsqueeze(0)).detach().numpy()
